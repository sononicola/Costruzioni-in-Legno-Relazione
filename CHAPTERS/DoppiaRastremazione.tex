...
\begin{table}[H]
    ...
    \begin{tabular}{cS[table-format=4.1] S[table-format=4.3] S[table-format=3.3]}
        \toprule
		%\multicolumn{4}{c}{Azioni di progetto nei punti di sezione}\\
        Punto & \multicolumn{1}{c}{$x$ [\si{\milli\metre}]} & \multicolumn{1}{c}{$M_d$ [\si{\kilo\newton\metre}]}& \multicolumn{1}{c}{$V_d$ [\si{\kilo\newton}]} \\
        \midrule
...
\begin{table}[H]
    ...
    \begin{tabular}{lS[table-format=2.1] @{\hspace{1cm}}l S[table-format=2.3]}
        \toprule
		\multicolumn{4}{c}{Valori geometrici di progetto}\\
        \midrule
		$b$      & \SI{!{b}}{\milli\metre}                      & $h_{ap}$        & \SI{!{h_ap}}{\milli\metre}   \\
		$h_0$    & \SI{!{h_0}}{\milli\metre}                    & $l$           & \SI{!{l}}{\milli\metre}   \\
		$\alpha$ & \SI{!{round(np.rad2deg(alpha),1)}}{\degree} & $\alpha_{ap}$ & \SI{!{round(np.rad2deg(alpha_ap),1)}}{\degree} \\
        \midrule
        \multicolumn{4}{c}{Valori di resistenza GL28h [\si{\mega\pascal}] } \\
        \midrule
        $f_{m,k}$    & !{f_mk}   & $f_{m,d}$    & !{round(f_md,3)}   \\
        $f_{v,k}$    & !{f_vk}   & $f_{v,d}$    & !{round(f_vd,3)}  \\
        $f_{c,90,k}$ & !{f_c90k} & $f_{c,90,d}$ & !{round(f_c90d,3)} \\
        $f_{t,90,k}$ & !{f_t90k} & $f_{t,90,d}$ & !{round(f_t90d,3)} \\
        $E_{0,mean}$ & !{E0mean} & $E_{0,05}$   & !{E005} \\
        $G_{mean}$   & !{Gmean}  & $G_{05}$     & !{G05}\\
        \bottomrule
    \end{tabular}
\end{table} 
